...
class SliderDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('banner',function($query){
                return $img="<img width='100' height='100' src='".asset($query->banner)."' ></img>";
            })
            ->addColumn('status',function($query){
                $active='<i class="badge badge-info">Aktif</i>';
                $inactive='<i class="badge badge-danger">Pasif</i>';
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('serial')->title('Seri'),
            Column::make('status')->title('Durum'),
            Column::computed('action')->title('İşlem')
                ->exportable(false)
                ->printable(true)
                ->width(250)
